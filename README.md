...
│   └── employee/                # 従業員用画面
│       ├── menu.php             # 従業員メニュー
│       ├── shifts.php           # 勤務可能日登録・シフト確認（勤務可能日登録・休み申請を統合）
│       ├── notifications.php    # 通知確認
...
パスワードは全ユーザー共通で `a`（開発用、`password_hash()` でハッシュ化して保存済み）です。

| ユーザーID | パスワード | role | 表示名 | ログイン後の遷移先 |
|---|---|---|---|---|
| manager01  | a | manager  | 小笠原秀人 | `pages/manager/menu.php`（店長メニュー） |
| employee01 | a | employee | LOVOT | `pages/employee/menu.php`（従業員メニュー） |
| employee02 | a | employee | 小室佑太 | `pages/employee/menu.php`（従業員メニュー） |
| employee03 | a | employee | 渡辺大智 | `pages/employee/menu.php`（従業員メニュー） |
| employee04 | a | employee | 塚本恭平 | `pages/employee/menu.php`（従業員メニュー） |
| employee05 | a | employee | 星野蒼 | `pages/employee/menu.php`（従業員メニュー） |
| employee06 | a | employee | 坂本実南 | `pages/employee/menu.php`（従業員メニュー） |
| employee07 | a | employee | 飯島凛 | `pages/employee/menu.php`（従業員メニュー） |
| employee08 | a | employee | 星野理一 | `pages/employee/menu.php`（従業員メニュー） |
| employee09 | a | employee | 石井颯斗 | `pages/employee/menu.php`（従業員メニュー） |
| employee10 | a | employee | 陶修斗 | `pages/employee/menu.php`（従業員メニュー） |
| employee11 | a | employee | 吉田隆人 | `pages/employee/menu.php`（従業員メニュー） |
| employee12 | a | employee | 川口将英 | `pages/employee/menu.php`（従業員メニュー） |
| employee13 | a | employee | 高木悠希 | `pages/employee/menu.php`（従業員メニュー） |
...
| `leave_approved` | 休み承認済み | 手動対応の代勤シフト登録が完了し、元の担当者を休み確定として残している状態。代勤者は `related_leave_request_id` で紐づく別シフトとして登録される | `scheduled`（休み申請者側キャンセル承認） |
...
| 状態名 | 表示名 | 意味 |
|---|---|---|
| `proposed` | 未回答 / 未通知 | 代勤候補として抽出された状態。`notified_at` がある場合は「未回答」、`notified_at` がない場合は「未通知」と表示する |
...
**重要：`accepted` は「店長承認済み」ではありません。** あくまで「候補者本人が代勤可能と回答した」状態です。店長の最終承認は `approvals` テーブルと `leave_requests.status = approved` / `shifts.status = substituted` で管理します。承認時、選ばれなかった `proposed` の候補者は `expired` に更新されます。一方、選ばれなかった `accepted` の候補者は、承認後に代勤者キャンセルが発生した場合のバックアップ候補として残します（`declined` の候補者もそのまま残ります）。
...
`leave_request` / `candidate_offer` という type は過去バージョンで使われていた古い種別で、現在のアプリケーションコードはこれらの type で新規通知を作成しません。`database/seed.sql` のサンプル通知も現行の `substitute_request` / `candidate_available` へ置き換え済みです（`app/includes/status_labels.php` には過去データ表示用ラベルのみ残しています）。新規実装でこの2種別を使わないでください。
...
* 従業員側画面（メニュー、勤務可能日登録・シフト確認、通知確認、代勤提案への応答、申請確認）
...
* 従業員側：勤務可能日登録・シフト確認（`pages/employee/shifts.php`）
  * シフト確認・勤務可能日登録・休み申請の3機能を1つの週表示シフト表に統合
  * シフト表上部の「＋ 勤務可能日を登録」ボタンから、勤務可能日をポップアップ登録できる（入力チェック：必須項目、開始時刻＜終了時刻、重複時間帯の登録防止）
  * ログイン中従業員の行を一番上に表示し、勤務可能日と実際のシフト（予定・代勤反映済み等）を同じシフト表上に表示する
  * シフト表内の自分の「予定」シフトを開くと、その場で休み申請ができる（申請の重複登録防止、登録直後に代勤候補抽出・通知作成を実行）
  * 店長処理前の申請（`matching` / `no_candidate`）は、同じ詳細ポップアップから本人がキャンセル可能（キャンセル時は関連候補者を無効化し、対象シフトを「予定」に戻す）
  * 承認済みで代勤担当になっているシフトからは、代勤キャンセル申請も送信できる
  * 他の従業員の勤務可能日・シフトは操作不可（`employee_id` で本人に限定）
* 代勤候補抽出・通知作成サービス（`app/services/substitute_matcher.php`）
  * 勤務可能日・時間帯が一致する従業員を代勤候補として `substitute_candidates` に登録
  * スコア上位3人の候補者へ「代勤依頼」通知、候補者がいない場合は店長へ「候補者なし」通知を作成
...
* 従業員側：申請確認（`pages/employee/result.php`）
  * 休み申請・代勤申請・休みキャンセル申請・代勤キャンセル申請を一覧表示
  * 表示上の非表示・お気に入りは `request_view_states` で管理し、申請履歴そのものは削除しない
  * 申請履歴の確認、表示上の非表示、お気に入り登録を行う
...
### 3. 従業員側：勤務可能日登録・シフト確認（`pages/employee/shifts.php`）

従来「シフト確認」「勤務可能日登録」「休み申請」の3画面に分かれていた機能を、1つの週表示シフト表に統合しています。

* 従業員メニューの「勤務可能日登録・シフト確認」から開きます。
* シフト表は週表示で、ログイン中従業員の行が一番上に表示されます。
* 自分のシフト（予定・代勤反映済み等）と勤務可能日が同じシフト表の中に並んで表示されます（凡例で色分けを確認できます）。
* シフト表上部の「＋ 勤務可能日を登録」ボタンを押すと、ポップアップで勤務可能日（日付・開始時刻・終了時刻・備考）を登録できます。
  * 入力チェック：勤務可能日・開始時刻・終了時刻は必須、開始時刻は終了時刻より前、同じ日付・時間帯に重複する登録は不可。
* 登録済みの勤務可能日イベントを開くと、詳細ポップアップから「🗑️ 削除する」で削除できます。
* 自分の「予定」シフトのイベントを開くと、詳細ポップアップから「休み申請をする」ボタンで休み申請できます（休み申請・代勤機能の詳細は後述）。
* 他の従業員のシフトも確認できますが、休み申請・勤務可能日削除などの操作は自分自身のデータに限定されます。

### 4. 店長側：シフト作成・一覧確認（`pages/manager/shifts.php`）

* 店長メニューの「シフト作成・一覧確認」から開きます。
* 「シフトの新規作成」フォームに、担当者（従業員または店長）・勤務日・開始時刻・終了時刻・担当業務・備考を入力して登録します。
* 店長シフトは `shifts.manager_user_id` で管理し、従業員の休み申請・代勤候補抽出の対象にはしません。
* 入力チェック：
  * 担当者・勤務日・開始時刻・終了時刻は必須です。
  * 開始時刻は終了時刻より前である必要があります。
  * 存在しない担当者、無効化された従業員は選択できません。
* エラーがある場合は同じ画面上にメッセージが表示されます。
* 登録済みシフトは週表示のシフト表で確認でき、「無効化」ボタンで論理削除（`status = 'cancelled'`）できます。
...
上記1〜6は**通常モード・スキル重視モードの必須条件**です。人員確保優先モードのみ、4の時間条件を「対象シフトと勤務可能時間が一部でも重なっている」に緩和し、人手を確保できる可能性を広げます。
...
2. スコア上位3人の候補者（の `users` アカウント）へ、種別 `substitute_request`・タイトル「代勤依頼が届いています」の通知を作成する。
3. 通知済み候補が辞退した場合は、空いた通知枠に次点候補を追加で通知する。
4. 休み申請（`leave_requests`）の `status` を `matching`（候補者回答待ち）に更新する。
...
### 従業員側：休み申請登録の使い方（`pages/employee/shifts.php`）

* 従業員メニューの「勤務可能日登録・シフト確認」のカレンダーから行います（専用の申請画面はなく、カレンダー内の詳細ポップアップから申請します）。
* カレンダー上の自分の「予定」シフトのイベントを開くと、詳細ポップアップに休み申請フォーム（申請理由の入力欄）が表示されます。「休み申請をする」を押すと申請できます。
* 申請すると、自動で代勤候補抽出が行われ、対象シフトのイベントの状態（候補者回答待ち／候補者なし（店長確認待ち）など）がカレンダー上に反映されます。
* 既に休み申請済みのシフトには申請フォームの代わりに現在の状態が表示されるため、重複申請はできません。
* 店長が承認・却下する前の申請には、同じ詳細ポップアップに「休み申請をキャンセルする」ボタンが表示されます。詳細は次の「休み申請キャンセル（承認前のみ）」を参照してください。
...
「休み申請をキャンセルする」を押すと、トランザクション内で以下を行います。
...
勤務可能日登録・シフト確認画面（`pages/employee/shifts.php`）のカレンダー詳細で、次の条件をすべて満たす場合に「承認後キャンセル申請を送る」フォームを表示します。
...
承認後は元の休み申請者の「勤務可能日登録・シフト確認」カレンダーに対象シフトが再表示され、代勤者のカレンダーからは対象シフトが外れます。
...
2. 代勤者が「勤務可能日登録・シフト確認」のカレンダーで対象代勤シフトを開き、「代勤キャンセル申請を送る」から申請する。
...
実装は `app/services/substitute_matcher.php` の `retrySubstituteMatching()` を中心に、`findSubstituteCandidatesForRetry()` / `getDeclinedEmployeeIdsForLeaveRequest()` / `getExistingCandidateStatus()` / `createOrReactivateCandidate()` / `createReopenedSubstituteRequestNotifications()` / `createRematchNoCandidateNotification()` に分離しています。

### 再抽出時の除外条件

通常の必須条件（有効な従業員・勤務可能日/時間条件・同時間帯にシフト重複なし）に加えて、以下を必ず除外します。時間条件は、通常モード・スキル重視モードでは対象シフトの完全カバー、人員確保優先モードでは対象シフトとの一部重なりを許可します。
...
* `proposed`：依頼中のため維持（通常は重複通知を作成しない）
...
新たに `proposed` になった候補者のうち、初めて通知対象になった候補者には既存の代勤依頼通知（`substitute_request`）を作成します。代勤者の承認後キャンセルによる再調整時は例外として、過去に通知済み・未回答だった候補者にも「代勤依頼が再度届いています」通知を新規作成します。これにより、空きシフトが復活したことに候補者が気づけるようにします。
...
2. Bが「勤務可能日登録・シフト確認」のカレンダーから代勤キャンセル申請を出し、店長がそれを承認する。
...
* `substitute_candidates.status` の値は既存定義（`proposed`/`accepted`/`declined`/`expired`）をそのまま利用しており、本機能では以下のように対応させています。
  * `proposed`：通知済みの場合は未回答、未通知の場合は未通知
...
* **未処理の休み申請**：休み申請者・勤務日・開始時刻・終了時刻・担当業務・ポジション・申請理由・状態（候補者回答待ち／候補者なし）と、代勤候補一覧（候補者名・回答状況・マッチ理由）を表示します。
  * 候補者は「代勤可能」「代勤不可」「未回答」「未通知」の4グループに分かれ、各グループに件数が表示されます。グループをクリックすると、候補者カードが下に展開されます。
  * 「未回答」は代勤依頼通知を送信済みだが、候補者がまだ回答していない状態です。「未通知」は代勤候補として条件を満たしているものの、上位3人の通知枠にまだ入っていない状態です。
  * 「代勤可能者から代勤登録」ボタンを押すと、代勤可能者の一覧がポップアップで表示されます。
  * ポップアップ内の候補者カードを押すと確認ダイアログが表示され、承認するとその候補者で代勤を確定します。
  * 未処理カード内の候補者カードは確認用で、直接の承認操作はできません。
  * 候補者が見つからない場合や、代勤可能な候補者がいない場合は、代勤承認操作はできません。
  * いずれの場合も「この休み申請を却下」ボタンから休み申請を却下できます。
* **処理済みの休み申請**：処理日時・休み申請者・対象シフト・結果（承認済み／却下）・代勤対応者（承認時のみ）を一覧表示します。

### 2. 承認時の処理

「代勤可能者から代勤登録」ボタンでポップアップを開き、候補者カードを押して確認ダイアログで承認すると、以下をトランザクション内で実行します。
...
4. 選ばれなかった未回答候補（`status = proposed`）を `expired` にする。「代勤可能」と回答済みだが選ばれなかった候補（`accepted`）は、承認後キャンセル時のバックアップ候補として残す。「代勤不可」と回答済み（`declined`）の候補者もそのまま残す。
...
### 5. 従業員側：申請確認画面の使い方（`pages/employee/result.php`）

* 従業員メニューの「申請確認」から開きます。
* 休み申請・代勤申請・休みキャンセル申請・代勤キャンセル申請を、日時の新しい順に一覧表示します。
* 代勤申請には、通知から代勤依頼を受け取り「代勤可能」と回答した履歴を表示します。
* 表示条件は「すべて」「休み申請」「代勤申請」「休みキャンセル申請」「代勤キャンセル申請」で切り替えできます。
* 各行には、対象シフト・申請種別・承認前/承認後の区分・現在のステータスを表示します。
* お気に入り登録、個別非表示、完了済み申請の一括非表示ができます。
* 非表示は `request_view_states` に保存される画面上の表示制御であり、`leave_requests` / `cancellation_requests` / `substitute_candidates` の履歴は削除しません。
* 完了済みで対象シフト日から90日以上経過した表示項目は、自動で非表示になります。ただしお気に入り登録済みの項目は自動非表示になりません。
...
抽出モードは、候補者のスコア計算と通知優先順位に使われます。条件を満たす候補者を抽出したうえで、スコア上位3人へ先に代勤依頼を通知し、辞退者が出た場合は次点候補へ段階的に通知します。
...
| モード | キー | 説明 |
|---|---|---|
| 通常 | `normal` | 勤務可能であることに加え、ポジション・スキル・勤続年数・時間一致度をバランスよく評価します。 |
| 人員確保優先 | `staffing_priority` | 一部の時間だけ勤務可能な従業員も候補に含め、人員確保の可能性を広げます。 |
| スキル重視 | `skill_priority` | 対象業務に対応できるスキルや経験を持つ従業員を優先します。 |
...
1. **必須条件**（`findSubstituteCandidates()`）：休み申請を出した本人ではない／従業員が有効（`is_active = 1`）／勤務可能日が一致／同時間帯の重複シフトがない。通常モード・スキル重視モードでは勤務可能時間が対象シフトを完全に覆っていることも必須です。人員確保優先モードでは、対象シフトと勤務可能時間が一部でも重なっていれば候補に含めます。
...
* **ポジション一致度**：候補者の担当可能ポジションが申請対象シフトの必要ポジションをすべて含むなら満点、必要ポジションの一部だけに対応できる場合は中間点、不一致なら0点。いずれかが未設定なら0点。
* **スキルレベル**：`employees.skill_level`（1〜5）に応じて、5→100%、4→80%、3→60%、2→40%、1→20%。
* **勤続年数**：`employees.hire_date` から、1年以上→満点、6か月以上→70%、3か月以上→40%、3か月未満→20%、未登録→0点。
* **時間一致度**：通常モード・スキル重視モードでは、候補者は対象シフトを完全にカバーしているため満点扱い。人員確保優先モードでは、対象シフトのうち勤務可能時間が何％重なっているかをスコア化します。勤務可能時間を広めに申告していても減点しません。

抽出理由（`match_reason`）は、上記の計算結果から「必要ポジション対応可、スキルレベル4、勤続1年以上、勤務可能時間が対象シフトを全てカバー」のような文言を自動生成し、`buildMatchReason()` で組み立てます。人員確保優先モードで部分的に勤務可能な候補者は「勤務可能時間が対象シフトの75%をカバー（一部時間のみ）」のように表示します。

### 候補者の並び順・通知の扱い

* 候補者は `createSubstituteCandidates()` 内でスコアの高い順に並べ替えてから `substitute_candidates` に登録します。
* 通知は、候補者全員ではなく、スコア上位3人にだけ送信します。
* 通知済み候補者が「代勤不可」、または店長承認前に代勤可能回答をキャンセルした場合、空いた通知枠に次点候補を追加で通知します。
* `substitute_candidates.notified_at` で通知済みかどうかを管理するため、通知一覧から通知を削除しても段階通知の判定は崩れません。
...
* 代勤候補一覧に「スコア」「抽出理由」「スキルレベル」「勤続」の列を追加し、候補者がどのような理由で選ばれたかを確認できるようにしました（例：候補者「鈴木次郎」／スコア「82点」／抽出理由「必要ポジション対応可、スキルレベル4、勤続1年以上、勤務可能時間が対象シフトを全てカバー」）。
...
パスワードはすべて `a` です。各状態は画面上に日本語の状態バッジで表示されます。
...
2. `employee01`（山田 太郎）でログインし、「勤務可能日登録・シフト確認」のカレンダーで「2026-06-14 17:00-22:00（キッチン）」のシフトを開き、休み申請する。状態が「候補者回答待ち」になる。
3. `employee03`（鈴木 次郎）でログインし、「通知確認」の代勤依頼から「回答する」→「代勤可能」を選ぶ。
4. `manager01` でログインし、「通知確認」の「代勤可能な候補者が回答しました」通知の「承認画面で確認する」リンクから承認画面へ移動し、「代勤可能者から代勤登録」ボタンを押して対象候補者カードを選ぶ。
5. `employee01` でログインし、「申請確認」で休み申請の状態を確認する。
6. `employee03` でログインし、「申請確認」で関連する申請状況を確認する。さらに「勤務可能日登録・シフト確認」のカレンダーに 2026-06-14 のシフトが「代勤シフト」として表示されることを確認する。
...
1. `employee04`（高橋 美咲）でログインし、「勤務可能日登録・シフト確認」のカレンダーで「2026-06-16 17:00-22:00（キッチン）」のシフトを開き、休み申請する。その日に勤務可能な他の従業員がいないため、状態が「候補者なし」になる。
...
2. `employee04` でログインし、「申請確認」で休み申請が却下済みになっていることを確認する。
...
4. 別の従業員（休み申請を出す側）でログインし、「勤務可能日登録・シフト確認」のカレンダーで対象シフトを開き、休み申請する。
...
* `seed.sql` のパスワードは開発用の共通パスワード（`a`）を `password_hash()` でハッシュ化したものです。
...
12. **従業員による勤務可能日登録・一覧確認**
    * 従業員メニューから「勤務可能日登録・シフト確認」を開く。
    * カレンダー上部の「＋ 勤務可能日を登録」ボタンを押し、ポップアップで勤務可能日・開始時刻・終了時刻・備考を入力して登録し、カレンダー上にイベントとして表示されることを確認する。
    * 開始時刻を終了時刻より後にして登録しようとすると、画面上にエラーメッセージが表示されることを確認する。
    * 同じ日付・時間帯と重複する内容を登録しようとすると、画面上にエラーメッセージが表示されることを確認する。
    * 登録した勤務可能日のイベントを開き、詳細ポップアップの「🗑️ 削除する」ボタンで削除できることを確認する。
13. **店長による勤務可能日確認**
...
15. **権限チェックの確認（追加分）**
    ...
    * 店長でログイン中に `pages/employee/shifts.php` に直接アクセスし、店長メニューへ転送されることを確認する。
    * 従業員でログイン中に、他の従業員の勤務可能日のレコードIDを指定して削除リクエストを送っても削除されないことを確認する（`employee_id` で本人のレコードに限定されているため）。
16. **休み申請登録・代勤候補抽出（候補者あり）の確認**
    * ユーザーID `employee01`（山田 太郎）でログインし、従業員メニューから「勤務可能日登録・シフト確認」を開く。
    * 対象シフトとして「2026-06-14 17:00-22:00（キッチン）」のイベントを開き、申請理由を入力して申請する。
...
17. **休み申請登録・代勤候補抽出（候補者なし）の確認**
    * ユーザーID `employee04`（高橋 美咲）でログインし、「勤務可能日登録・シフト確認」で自分のシフト「2026-06-16 17:00-22:00（キッチン）」のイベントを開き、休み申請する。
    * イベントの状態が「候補者なし（店長確認待ち）」と表示されることを確認する。
    * ログアウトし、ユーザーID `manager01` でログインして「通知確認」を開き、種別「候補者なし」・タイトル「代勤候補が見つかりません」の通知が表示されることを確認する。
18. **休み申請の重複防止の確認**
    * 手順16で申請したシフトを再度カレンダーで開こうとしても、休み申請フォームの代わりに現在の申請状態が表示され、二重申請できないことを確認する（対象シフトの状態が「予定」から変わっているため）。
...
23. **承認画面の表示・「承認画面で確認する」リンクの確認**
    ...
    * 「未処理の休み申請」に、休み申請者、勤務日、開始時刻、終了時刻、状態「候補者回答待ち」と、代勤候補の「代勤可能」「代勤不可」「未回答」「未通知」グループが表示されることを確認する。
    * 「代勤可能」グループを開くと、代勤可能と回答済みの候補者カードが確認用として表示されることを確認する。
    * 「代勤可能者から代勤登録」ボタンを押すと、代勤可能者の一覧がポップアップで表示されることを確認する。
24. **休み申請の承認（代勤確定）の確認**
    * 手順23に続けて、ポップアップ内の代勤可能な候補者カードを押し、確認ダイアログで承認する。
    * 「休み申請を承認し、代勤を確定しました。」と表示され、対象の休み申請が「未処理の休み申請」から消え、「処理済みの休み申請」に結果「承認済み」・代勤対応者「鈴木 次郎」として表示されることを確認する。
    * ログアウトし、ユーザーID `employee02`（佐藤 花子）でログインして「申請確認」を開き、勤務日「2026-06-15」の休み申請が表示されることを確認する。
    * ログアウトし、ユーザーID `employee03`（鈴木 次郎）でログインして「申請確認」を開き、関連する申請状況が表示されることを確認する。
25. **候補者なしの休み申請の却下の確認**
    * ユーザーID `employee04`（高橋 美咲）でログインし、「勤務可能日登録・シフト確認」で自分のシフト「2026-06-16 17:00-22:00（キッチン）」のイベントを開き、休み申請し、状態「候補者なし（店長確認待ち）」になることを確認する（手順17と同様）。
    ...
    * ログアウトし、ユーザーID `employee04`（高橋 美咲）でログインして「申請確認」を開き、勤務日「2026-06-16」の休み申請が却下済みとして表示されることを確認する。
...
31. **候補者ありの休み申請キャンセル**
    ...
    * カレンダーで同じシフトのイベントを開き、「休み申請をキャンセルする」を押し、状態が「キャンセル済み」になり、キャンセルボタンが消えることを確認する。
...
35. **承認後キャンセル申請の承認**
    ...
    * 休み申請者で「勤務可能日登録・シフト確認」を開き、該当シフトの詳細からキャンセル理由を入力して「承認後キャンセル申請を送る」を押す。
