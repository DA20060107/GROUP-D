...
将来は `request_type = substitute_after_approval` を追加し、代勤者側のキャンセル申請にも拡張できる構造です。今回は代勤者側キャンセル処理は実装していません。
...
| `cancelled_after_approval` | 承認後キャンセル済み（店長が承認後キャンセル申請を承認し、元の担当者へ戻した） |
...
| | substituted | 代勤反映済み |
| | cancelled | キャンセル |
| キャンセル申請（cancellation_requests） | pending | キャンセル申請中 |
| | approved | キャンセル承認済み |
| | rejected | キャンセル却下済み |
