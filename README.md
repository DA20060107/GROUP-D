...
* **未処理の休み申請**：休み申請者・勤務日・開始時刻・終了時刻・担当業務・ポジション・申請理由・状態（候補者回答待ち／候補者なし）と、代勤候補一覧（候補者名・回答状況・マッチ理由）を表示します。
  ...
  * 「代勤可能」グループ内の候補者カードを押すと確認ダイアログが表示され、承認するとその候補者で代勤を確定します。
  * 「代勤不可」「未回答」「未通知」のカードは確認用で、承認操作はできません。
...
「代勤可能」グループ内の候補者カードを押して確認ダイアログで承認すると、以下をトランザクション内で実行します。
...
4. `manager01` でログインし、「通知確認」の「代勤可能な候補者が回答しました」通知の「承認画面で確認する」リンクから承認画面へ移動し、「代勤可能」グループを開いて対象候補者カードを押す。
...
23. **承認画面の表示・「承認画面で確認する」リンクの確認**
    ...
    * 「代勤可能」グループを開くと、代勤可能と回答済みの候補者カードが表示されることを確認する。
24. **休み申請の承認（代勤確定）の確認**
    * 手順23に続けて、代勤可能な候補者カードを押し、確認ダイアログで承認する。
