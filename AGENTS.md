...
14. 従業員が申請確認画面で休み申請・代勤申請・キャンセル申請の状況を確認する
...
* 従業員メニュー
* 勤務可能日登録・シフト確認（カレンダー統合画面：自分のシフト確認・勤務可能日登録/一覧/削除・休み申請/一覧をすべて1画面で行う）
* 通知確認
* 代勤提案への応答
* 申請確認
...
leave_approved        : 休み承認済み（手動対応の代勤シフト登録が完了し、元の担当者を休み確定として残している状態。
                        手動登録された代勤シフトは related_leave_request_id で休み申請へ紐づく）
...
承認時、選ばれなかった `proposed` の候補者は `expired` に更新されます。選ばれなかった `accepted` の候補者は、承認後に代勤者キャンセルが発生した場合のバックアップ候補として残します（`declined` もそのまま残す）。
...
代勤者の承認後キャンセルによる再調整時は、過去に通知済み・未回答だった候補者にも `substitute_request` の再通知を作成します。空いた通知枠に入る新規候補者への通知も、通常の段階通知として作成します。
...
pages/employee/shifts.php                 # 勤務可能日登録・シフト確認（旧 shifts.php/availability.php/leave_request.php を
                                           # 1つのカレンダー画面に統合。勤務可能日登録・休み申請・承認前キャンセル・
                                           # 代勤者側キャンセル申請フォームをすべてこの画面のカレンダー詳細ポップアップで扱う）
pages/employee/notifications.php
pages/employee/candidate_response.php
pages/employee/result.php                 # 申請確認（休み申請・代勤申請・休みキャンセル申請・代勤キャンセル申請の表示状態管理）
```

`pages/employee/availability.php` と `pages/employee/leave_request.php` は統合により削除済み。今後これらのファイル名を復活させたり、新規に別画面として再作成したりしないこと。
...
1. `manager01 / a` でログインし、シフト一覧を確認
2. `employee01 / a` でログインし、2026-06-14の対象シフトに休み申請
3. `employee03 / a` でログインし、通知確認から代勤依頼に「代勤可能」と回答
4. `manager01 / a` でログインし、承認画面から代勤候補を承認
5. `employee01 / a` でログインし、申請確認画面で結果確認
6. `employee03 / a` でログインし、申請確認画面で結果確認
7. `manager01 / a` でログインし、シフト一覧で担当者が変更されていることを確認
...
代勤依頼通知は全候補者へ同時送信せず、`match_score` の高い上位3人へ先に送信します。通知済み候補者が `declined` になった場合、または店長承認前に `accepted` 回答をキャンセルした場合は、空いた通知枠に次点候補を追加通知します。代勤者の承認後キャンセルで空きシフトが復活した場合は、過去に通知済み・未回答だった候補者へ再通知し、候補者減少で空いた枠には次点候補を追加通知します。通知済み判定は `substitute_candidates.notified_at` で管理します。

**この機能で実装していないもの**（勝手に追加実装しないこと）：`proposal_round`/`reserved` 状態の導入、複数スキルテーブル、店長による重みの手動調整。
